...
def update_startup_screen():
    ...
    image1, width1, height1, draw1 = init_screen(color=config.WHITE)
    ...
    draw1.text(
        ...
        fill=config.BLACK,
        font=utils.create_font("sawasdee", 40),
    )
    draw.text(
        (14, 95),
    ...
    config.WAVESHARE.display(
        config.WAVESHARE.getbuffer(image), config.WAVESHARE.getbuffer(image1)
    )
...
def update_qr_request():
    ...
    for i in range(0, 3):
        ...
        config.WAVESHARE.init()
        config.WAVESHARE.display(
            config.WAVESHARE.getbuffer(image), config.WAVESHARE.getbuffer(image)
        )
